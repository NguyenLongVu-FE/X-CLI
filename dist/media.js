...
export async function describeMedia(paths) {
...
export async function verifyMedia(descriptors) {
    for (const descriptor of descriptors) {
        try {
            const metadata = await stat(descriptor.path);
            const hash = await sha256File(descriptor.path);
            if (!metadata.isFile() || metadata.size !== descriptor.size || hash !== descriptor.sha256)
