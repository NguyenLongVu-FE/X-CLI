...
describe('browser process lock', () => {
  ...
  it('preserves a stale lock instead of risking deletion of a replacement owner', async () => {
    ...
    expect(JSON.parse(await readFile(path, 'utf8'))).toEqual({ pid: 999, startedAt: 1, token: 'stale' });
  });
