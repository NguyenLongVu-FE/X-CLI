...
const postTarget = (kind) => z.strictObject({ kind: z.literal(kind), postId });
const userTarget = (kind) => z.strictObject({ kind: z.literal(kind), username });
...
export const BulkInputSchema = z.strictObject({
    ...
    for (const [index, action] of input.actions.entries()) {
        ...
        if (seen.has(canonical))
            context.addIssue({ code: 'custom', path: ['actions', index], message: 'Duplicate bulk action' });
        seen.add(canonical);
    }
});
